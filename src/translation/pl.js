export const pl = {
	'link-about': 'O nas',
	'link-pricing': 'Ceny',
	'link-contacts': 'Kontakty',

	'hero-title': 'Brwi, Rzęsy i&nbsp;Paznokcie',
	'hero-subtitle':
		'Ucieknij od codziennej rzeczywistości i zadbaj o swoją skórę',

	'appoint-name': 'imię',
	'appoint-phone': 'telefon',
	'appoint-date': 'date',
	'appoint-btn': 'Umów się na wizytę',

	'featured-title': 'Najnowsze Oferty',
	'featured-subtitle':
		'Oferujemy kompleksową gamę usług paznokciowych, obejmującą stylizację, koloryzację, barwienie brwi i rzęs, depilację i wiele innych.',
	'featured-btn': 'zarezerwuj teraz',
	'featured-nails': 'Paznokcie',
	'featured-any': 'Wszystkie kolory, jakie sobie wyobrażasz',
	'featured-brows': 'Brwi i Rzęsy',
	'featured-create': 'Tworzenie Twojego Wizerunku',
	'featured-depilat': 'Depilacja',
	'featured-treat': 'Zadbaj o swoją skórę',

	'promo-hidden': 'promocyjna zniżka',
	'promo-subtitle': 'tylko 3 dni',
	'promo-title': '20% zniżki na usługi stylizacji paznokci',
	'promo-btn': 'zobacz więcej',

	'portfolio-title': 'Nasze Portfolio',
	'portfolio-subtitle':
		'Sprawdź pełne portfolio naszych prac, obejmujące paznokcie, brwi, rzęsy i wiele więcej. Wszystko, co widzisz tutaj, zostało wykonane przez nasz zespół.',

	'about-title': 'O Nas',
	'about-top':
		'Beauty Style to jeden z najlepszych salonów urody w Gdańsku, odwiedzany nie tylko przez Polaków, ale także przez Ukraińców, Niemców i przedstawicieli wielu innych krajów.',
	'about-bottom':
		'Nasz salon urody zyskał niesamowitą reputację, ponieważ nasz profesjonalny zespół nadal zdumiewa klientów poprzez pracę nad oczami i paznokciami, podkreślając ich atuty za pomocą naszych usług',
	'about-btn': 'więcej informacji',

	'equipment-hidden': 'Nasze zalety',
	'equipment-team': 'Przyjazny Zespół',
	'equipment-staff':
		'Nasz personel jest uprzejmy i grzeczny. Dążymy do doskonałości zarówno pod względem jakości naszych usług, jak i interakcji z Tobą.',
	'equipment-equip': 'Najlepszy Sprzęt',
	'equipment-best':
		'Korzystamy z najlepszego sprzętu i produktów, aby świadczyć najwyższy poziom usług kosmetycznych naszym klientom',
	'equipment-sphere': 'Doskonała Atmosfera',
	'equipment-atmosphere':
		'Kiedy przychodzisz do Beauty Style, masz gwarancję doświadczenia ciepłej i przyjaznej atmosfery naszego salonu.',

	'pricing-title': 'Cennik',
	'pricing-lash&brows': 'Rzęsy i Brwi',
	'pricing-femail': 'Depilacja kobieta',
	'pricing-mail': 'Depilacja mężczyzną',
	'pricing-manicure': 'Manicure & Pedicure',
	'pricing-lashes': 'Rzęsy',
	'pricing-laminatelashes': 'LAMINACJA RZĘS',
	'pricing-botox': 'LAMINACJA RZĘS + KOLOR + BOTOX',
	'pricing-extense': 'PRZEDŁUŻENIE RZĘS',
	'pricing-wet': 'MOKRY EFEKT',
	'pricing-kim': 'PRZEDŁUŻENIE RZĘS EFEKT KIM',
	'pricing-tips': 'PRZEDŁUŻENIE RZĘS KONCÓWEK',
	'pricing-remove': 'USUWANIE PRZEDŁUŻONYCH RZĘS',

	'pricing-eyebrows': 'BRWI',
	'pricing-shaping': 'REGULACJA BRWI',
	'pricing-laminatebrows': 'LAMINACJA BRWI',
	'pricing-correction': 'LAMINACJA, KOLORYZACJA I KOREKCJA BRWI',

	'pricing-femdepilat': 'DEPILACJA KOBIETA',
	'pricing-sideburns': 'BAKI',
	'pricing-nose': 'NOS',
	'pricing-chin': 'BRODA',
	'pricing-neck': 'SZYJA',
	'pricing-armpits': 'PACHY',
	'pricing-arms': 'RĘCE',
	'pricing-line': 'LINIA NA BRUCHU',
	'pricing-lumbar': 'LĘDŹWIE',
	'pricing-legs': 'NOGI CAŁE',
	'pricing-bikinitop': 'BIKINI TRADYCYNE',
	'pricing-bikinideep': 'BIKINI GŁĘBOKE',
	'pricing-butt': 'POŚLADKI',

	'pricing-maildepilat': 'Depilacja mężczyzną',
	'pricing-ears': 'USZY',
	'pricing-back': 'PLECY',
	'pricing-back+arms': 'PLECY + RAMIONA',
	'pricing-tors': 'TORS',
	'pricing-stomack': 'BRZUCH',
	'pricing-torsostomack': 'TORS + BRZUCH',
	'pricing-maillegs': 'NOGI',
	'pricing-bikinilegs': 'BIKINI + POŚLADKI',

	'pricing-manicuretitle': 'Manicure & Pedicure',
	'pricing-hybridmanicure': 'MANICURE HYBRYDOWY',
	'pricing-hybridpicture': 'MANICURE HYBRYDOWY + OBRAZEK',
	'pricing-conditioner': 'MANICURE Z ODŻYWKĄ',
	'pricing-french': 'FRENCH',
	'pricing-gel': 'PRZEDŁUŻENIE ŻELEM + HYBRYDA',
	'pricing-natural': 'ŻEL NA NATURALNE PŁYTCE',
	'pricing-picturehybrid': 'ZDJĘCIE HYBRYDY I MANICURE Z ODŻYWKĄ',
	'pricing-gelpicture': 'ZDJĘCIE HYBRYDY I MANICURE Z ODŻYWKĄ',
	'pricing-hybridpedicure': 'PEDICURE HYBRYDOWY',
	'pricing-pedicurefeet': 'PEDICURE HYBRYDOWY + STOPY',
	'pricing-pedicurecondicioner': 'PEDICURE Z ODŻYWKĄ',
	'pricing-reconstruction': 'REKONSTRUKCJA PAZNOKCIA W NASZYM SALONIE',
	'pricing-gelremove': 'SCIĄGANIE ŻELU (USŁUGA DODANA DO INNEJ)',

	'recommends-title': 'Informacja dla klienta',
	'recommends-check': 'Lista kontrolna idealnego klienta',
	'recommends-late':
		'Staraj się nie spóźniać więcej niż 15 minut (jeśli spóźnisz się więcej niż 15 minut, spotkanie zostanie odwołane lub przełożone).',
	'recommends-time':
		'Jeśli się spóźnisz, daj nam znać, ponieważ czas dla każdego klienta jest ograniczony.',
	'recommends-alone': 'Przyjdź bez opieki.',
	'recommends-sick':
		'Jeśli zachorujesz, pamiętaj, aby ostrzec i przełożyć wizytę.',
	'recommends-plan':
		'Pamiętaj, że wysokiej jakości praca wymaga czasu i cierpliwości, zaopatrz się w nią z wyprzedzeniem.',

	'recommends-trim': 'Dlaczego nie możesz obciąć brwi',
	'recommends-hair': 'Z natury włosy mają cienką, uszczelnioną końcówkę.',
	'recommends-disrupts':
		'Przycinanie brwi zaburza ich strukturę. Grube, strzyżone włosy nie układają się dobrze i zwisają, co z czasem może prowadzić do zmiany kierunku wzrostu.',
	'recommends-structure':
		'Ze względu na zmianę struktury w wyniku farbowania, kolor brwi będzie niejednorodny. Przycięte włosy będą jaśniejsze, bez cienkiej końcówki.',
	'recommends-dye':
		'Jednocześnie pigment wyjdzie z nich szybciej, co skraca trwałość farby.',
	'recommends-fence':
		'Prosta linia strzyżenia będzie wyglądać jak płot za tydzień, ponieważ włosy przechodzą przez różne fazy wzrostu.',
	'recommends-whiskers': 'W dotyku włosy będą grube i kłujące, podobно wąsom.',

	'recommends-depilation': 'Depilacją',
	'recommends-before':
		'Przed depilacją należy przygotować skórę tak, aby depilacja była skuteczna i jak najmniej traumatyczna. Niektóre etapy przygotowawcze należy rozpocząć na kilka dni, a nawet tygodni przed zabiegiem. Zaplanuj zabieg depilacji przynajmniej na miesiąc wcześniej, aby mieć pewność, że Twoja skóra pozostanie zdrowa i gładka.',
	'recommends-predepilation': 'Zalecenia przed zabiegiem depilacji:',
	'recommends-length':
		'Długość włosów przed pierwszą depilacją wynosi co najmniej 6-7 mm. To około 2 tygodnie po goleniu.',
	'recommends-subsequent':
		'Do kolejnych zabiegów wystarczy 5-6 mm, ponieważ włosy będą cieńsze i łatwiejsze do usunięcia.',
	'recommends-peels':
		'Na trzy dni przed zabiegiem nie zaleca się wykonywania peelingów.',
	'recommends-sunbathe':
		'Nie zaleca się opalania się na słońcu i odwiedzania solarium przez siedem dni.',
	'recommends-oils':
		'W dniu zabiegu nie należy stosować tłustych balsamów ani olejków.',
	'recommends-razor':
		'Nigdy nie używaj maszynki do golenia, kremu do depilacji ani depilatora między zabiegami.',

	'recommends-contraindications': 'Przeciwwskazania do zabiegu. Obecność:',
	'recommends-burns':
		'Оtwartych ran, krost, skaleczeń, oparzeń (w tym oparzeń słonecznych) na skórze.',
	'recommends-bypass':
		'W niektórych przypadkach możemy ominąć obszar, w którym występują uszkodzenia i wykonać zabieg.',
	'recommends-varicose': 'Wyraźne żylaki.',
	'recommends-pregnancy': 'Pierwszy i ostatni trymestr ciąży',

	'recommends-postdepilation': 'Zalecenia po zabiegu depilacji:',
	'recommends-sauna':
		'Pierwszego dnia nie należy brać gorącego prysznica (kąpieli) oraz nie korzystać z sauny.',
	'recommends-prohibit': 'Surowo zabrania się używania myjki pierwszego dnia.',
	'recommends-sports': 'Nie uprawiaj sportu.',
	'recommends-clothing':
		'Następnego dnia po depilacji woskiem załóż wygodną bawełnianą bieliznę i luźną, oddychającą odzież.',
	'recommends-deodorant':
		'Nie nakładaj dezodorantu na depilowane miejsca. Surowo zabrania się opalania się na słońcu i korzystania z solarium.',
	'recommends-scrub':
		'Nie szoruj depilowanych miejsc przez pierwsze trzy dni po zabiegu.',

	'recommends-goal':
		'Po kilku zabiegach włosy rosną znacznie wolniej i stają się cieńsze i słabsze, a to również wpływa na czułość zabiegu! Celem depilacji woskiem jest nie tylko usuwanie włosów, ale stopniowa zmiana ich jakości i ilości.',
	'recommends-difference': 'Po 4-5 zabiegach zauważysz ogromną różnicę:',
	'recommends-softer': 'Włosy stają się bardziej miękkie i jaśniejsze.',
	'recommends-remnants':
		'Resztki włosów nie mają szorstkiego cięcia, jak po brzytwie.',
	'recommends-tolerated':
		'Skóra staje się bardziej podatna na depilację, a zabieg jest łatwiej tolerowany.',
	'recommends-shortened':
		'Ze względu na to, że włosów jest mniej, skraca się również czas zabiegu.',
	'recommends-silky': 'Skóra staje się gładka, elastyczna i jedwabista.',

	'work-title': 'Kontakt i godziny otwarcia',
	'work-monday': 'poniedziałek',
	'work-tuesday': 'wtorek',
	'work-wednesday ': 'środa ',
	'work-thursday': 'czwartek',
	'work-friday': 'piątek',
	'work-saturday': 'sobota',
	'work-sunday': 'niedziela',
	'work-btn': 'Zadzwoń',

	'contacts-title': 'Kontakty',
	'contacts-subtitle': 'Możesz do nas dzwonić o każdej porze',
	'contacts-address': 'Lawendowa 8, 80-840 Gdańsk, Polska',
	'contacts-emailus': 'Nie wahaj się wysłać nam swoich pytań drogą mailową',
	'contacts-touch': 'SKONTAKTUJ SIĘ',
	'contacts-mistake': 'To pole jest wymagane',
	'contacts-btn': 'wyślij wiadomość',

	'footer-hidden': 'stopka strony',
	'footer-pricing': 'Cennik',
	'footer-contacts': 'Kontakty',
	'footer-gallery': 'Galeria',
	'footer-about': 'O Nas',
};
